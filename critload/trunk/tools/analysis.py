...
nero_ag  = np.loadtxt("nero_ag.asc"       , skiprows=6)
nero_nat = np.loadtxt("nero_nat.asc"      , skiprows=6)
...
nle_crit_gw_ara = np.loadtxt("nle_crit_gw_ara.asc" , skiprows=6)
nle_crit_gw_igl = np.loadtxt("nle_crit_gw_igl.asc" , skiprows=6)
...
nero_ag[nero_ag==-9999]                 =np.nan
nero_nat[nero_nat==-9999]               =np.nan
...
nle_crit_gw_ara[nle_crit_gw_ara==-9999] =np.nan 
nle_crit_gw_igl[nle_crit_gw_igl==-9999] =np.nan 
...
with np.errstate(invalid='ignore'):
    print("total-area: %i"% np.nansum(a_araigl))
    print("case-1: %i" % np.nansum(a_araigl[np.where(noxem>ndepcrit)]))
    print("case-2: %i" % np.nansum(a_araigl[np.where(nh3em_egl>ndepcrit)]))
    print("case-3: %i" % np.nansum(a_araigl[np.where(noxem_nh3em_egl>ndepcrit)]))
...
# calcualte % area for different cases
print("CASES surfacewater")
with np.errstate(invalid='ignore'):
    print("total-area: %i"% np.nansum(a_araigl))
    print("case-1:  %i"   % np.nansum(a_araigl[np.where(npoint>nload_crit)]))
    print("case-1a: %i"   % np.nansum(a_araigl[np.where(nww>nload_crit)]))
    print("case-1b: %i"   % np.nansum(a_araigl[np.where(naqua>nload_crit)]))
    print("case-1c: %i"   % np.nansum(a_araigl[np.where(ndep_sw>nload_crit)]))
    print("case-1d: %i"   % np.nansum(a_araigl[np.where(nallo>nload_crit)]))
    print("case-2:  %i"   % np.nansum(a_araigl[np.where(nero>nload_crit)]))
    print("case-2a: %i"   % np.nansum(a_araigl[np.where(nero_ag>nload_crit)]))
    print("case-2b: %i"   % np.nansum(a_araigl[np.where(nero_nat>nload_crit)]))
    print("case-3:  %i"   % np.nansum(a_araigl[np.where(nload_fixed_ag>nload_crit)]))
    print("case-4:  %i"   % np.nansum(a_araigl[np.where(nload_fixed_nat>nload_crit)]))
    print("case-5:  %i"   % np.nansum(a_araigl[np.where(nload_fixed>nload_crit)]))
    print("case-6:  %i"   % np.nansum(a_araigl[np.where(nero_npoint>nload_crit)]))
    print("case-7:  %i"   % np.nansum(a_araigl[np.where(nload_tot>nload_crit)]))
...
'''
###### GROUNDWATER ##########
nle_crit_gw = np.add(nle_crit_gw_ara, nle_crit_gw_igl)
...
with np.errstate(invalid='ignore'):
    print("total-area: %i"% np.nansum(a_araigl))
    print("case-1: %i" % np.nansum(a_araigl[np.where(nle_nfix_gw   >nle_crit_gw)]))
    print("case-2: %i" % np.nansum(a_araigl[np.where(nle_nox_gw    >nle_crit_gw)]))
    print("case-3: %i" % np.nansum(a_araigl[np.where(nle_nh3egl_gw >nle_crit_gw)]))
    print("case-4: %i" % np.nansum(a_araigl[np.where(nle_fix_all   >nle_crit_gw)]))
    
'''
# calculate percentages
nle_nfix_per        = np.divide(nle_nfix_gw,nle_crit_gw)
nle_nox_per         = np.divide(nle_nox_gw,nle_crit_gw)
nle_nh3egl_per      = np.divide(nle_nh3egl_gw,nle_crit_gw)    
nle_fix_all_per = np.divide(nle_fix_all,nle_crit_gw)
